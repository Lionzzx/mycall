<template>
	<view class="register">
		<view class="content">
			...
			<wButton style="margin-top: 100upx;" text="登 录" :rotate="isRotate" @click.native="startReg()"></wButton>

			<!-- 底部信息 -->
			<view class="footer">
				<text @tap="isShowAgree" class="cuIcon" :class="showAgree ? 'cuIcon-radiobox' : 'cuIcon-round'">同意</text>
				<!-- 协议地址 -->
				<navigator url="" open-type="navigate">《体验版》</navigator>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		async getVerCode() {
			...
			const resp = await getCode(_this.phoneData);
			console.log('获取验证码', resp);
			this.$refs.runCode.$emit('runCode', 60); //触发倒计时（一般用于请求成功验证码后调用）
		},
		async startReg() {
			//注册
			if (this.isRotate) {
			...
			_this.isRotate = true;
			try {
				const resp = await login({ phone: this.phoneData, password: this.verCode });
